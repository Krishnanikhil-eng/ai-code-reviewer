...
async def github_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    if event_type == "pull_request":
        ...
        if action in ["opened", "synchronize"]:
            ...
            return {
                "status": "accepted",
                "message": f"Processing PR #{pr_number} securely.",
            }
    ...
    return {
        "status": "ignored",
        "message": f"Event '{event_type}' ignored, listening only for target events.",
    }
...
def get_stats():
    ...
    try:
        with get_connection() as conn:
            ...
            scored = conn.execute(
                "SELECT COUNT(*) FROM ai_comments WHERE score != 0"
            ).fetchone()[0]
            ...
            repos = conn.execute(
                "SELECT COUNT(DISTINCT repo_full_name) FROM ai_comments"
            ).fetchone()[0]
...
def get_reviews(
    ...
    if search:
        ...
        count_query += (
            " AND (code_snippet LIKE ? OR comment_text LIKE ? OR file_path LIKE ?)"
        )
...
async def save_settings(request: Request):
    ...
    success = save_repo_settings(
        repo, strictness, review_mode, custom_prompt, retrieval_depth
    )
    if success:
        log_action(
            role,
            f"Updated settings for {repo}",
            f"mode={review_mode}, strictness={strictness}",
        )
...
async def get_status():
    ...
    try:
        response = requests.get(
            settings.OLLAMA_API_URL.replace("/api/generate", ""), timeout=2
        )
...
def trigger_retrain(role: str = "Developer"):
    if role != "Admin":
        raise HTTPException(
            status_code=403, detail="Only Admins can trigger retraining."
        )
...
async def delete_memory(request: Request):
    ...
    if role != "Admin":
        raise HTTPException(
            status_code=403, detail="Only Admins can modify RAG memory."
        )
...
async def boost_memory(request: Request):
    ...
    if role != "Admin":
        raise HTTPException(
            status_code=403, detail="Only Admins can boost vector memory."
        )
    ...
    try:
        ...
        log_action(
            role,
            "Boosted vector memory score",
            f"ID: {memory_id}, New Score: {boost_score:.2f}",
        )
        await manager.broadcast({"event": "refresh"})
        return {"status": "success"}
    except Exception as e:
