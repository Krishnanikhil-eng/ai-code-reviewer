...
def get_installation_client(installation_id: int) -> Github:
    ...
    """
    if not settings.GITHUB_PRIVATE_KEY_PATH or not os.path.exists(
        settings.GITHUB_PRIVATE_KEY_PATH
    ):
        logger.warning(f"Private key not found at {settings.GITHUB_PRIVATE_KEY_PATH}.")
        logger.warning(
            "Failing back to unauthenticated client (useful only for public repos and testing PRs without pushing comments)."
        )
        return Github()
    ...
    try:
        ...
        app_auth = Auth.AppAuth(
            app_id=settings.GITHUB_APP_IDENTIFIER, private_key=private_key
        )
        ...
        logger.error(
            f"Failed to authenticate GitHub App with installation ID {installation_id}: {e}"
        )
        return Github()


def post_pr_comment(
    repo_full_name: str, pr_number: int, installation_id: int, comment: str
) -> int:
    """
