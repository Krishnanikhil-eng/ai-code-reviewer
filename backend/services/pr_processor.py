...
def is_dataset_empty() -> bool:
    ...
    data_file_path = os.path.normpath(
        os.path.join(base_dir, "../../database/training_data.json")
    )
...
def process_pull_request(repo_full_name: str, pr_number: int, installation_id: int):
    ...
    try:
        ...
        if is_dataset_empty():
            logger.info(
                "Training dataset is empty. Triggering historical PR scraping for initial context."
            )
        ...
        logger.info(
            f"Fetched PR #{pr_number}. Total files changed: {diff_files.totalCount}"
        )

        for file in diff_files:
            logger.info(
                f"File modified: {file.filename} (+{file.additions} | -{file.deletions} lines)"
            )
            if file.patch:
        ...
        logger.info(
            f"Successfully processed diffs for PR #{pr_number}. Starting AI review..."
        )
        ...
        for file in diff_files:
            ...
            if comment_text and comment_text != "No comment provided.":
                ...
                comment_id = post_pr_comment(
                    repo_full_name, pr_number, installation_id, full_comment
                )
                if comment_id:
        ...
        logger.info(
            f"AI review complete. Posted {comments_posted} comments for PR #{pr_number}."
        )
