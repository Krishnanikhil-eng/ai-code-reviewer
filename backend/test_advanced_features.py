...
from backend.core.database import init_db, get_connection  # noqa: E402
from retrain import run_retraining  # noqa: E402
from vector_store.chroma_client import ensure_collection  # noqa: E402
from backend.services.reaction_handler import _detect_sentiment  # noqa: E402
from backend.services.dataset_builder import DatasetBuilder  # noqa: E402
...
class TestAdvancedFeatures(unittest.TestCase):
    ...
    def test_score_decay_calculation(self):
        ...
        ten_days_ago = (datetime.utcnow() - timedelta(days=10)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        print(
            f"Decayed Score: Expected = {expected_decayed_score:.4f}, Stored = {stored_score:.4f}"
        )
    ...
    def test_llm_assisted_sentiment_fallback(self, mock_post):
        ...
        print(
            f"LLM Fallback Sentiment (Neutral input): Expected = -1, Returned = {sentiment}"
        )
